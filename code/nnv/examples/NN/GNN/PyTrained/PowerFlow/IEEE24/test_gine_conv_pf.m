% test_gine_conv_pf.m - Test Python-trained full GINE model via gnn2nnv
%
% Demonstrates the Python -> MATLAB import pipeline for full GINE:
%   1. Load .mat exported from Python training
%   2. Import with gnn2nnv() (requires GINEConvLayer)
%   3. Verify evaluate() matches Python predictions
%   4. Run reachability analysis (node perturbation)
%
% Note: Requires GINEConvLayer.m from feature/gine-conv-layer branch.
%
% Author: Anne Tumlin
% Date: 02/11/2026
...
modelPath = fullfile(fileparts(mfilename('fullpath')), 'models', 'e2e_gine_conv.mat');
fprintf('=== Python-trained Full GINE (GINEConvLayer) Test ===\n');
fprintf('Model: %s\n\n', modelPath);
