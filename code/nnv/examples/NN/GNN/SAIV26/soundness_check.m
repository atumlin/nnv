%% soundness_check.m
% Soundness validation for ALL PF/OPF GNN verification pipelines.
%
% Tests all 4 architectures x 3 grids x 2 tasks = 24 model configs.
% For each config, loads the model, runs reachability on a few graphs,
% then Monte Carlo samples random perturbations to confirm that every
% concrete GNN evaluation lies within the computed reachable bounds.
%
% Checks:
...
addParameter(p, 'grids', {'IEEE24', 'IEEE39', 'IEEE118'}, @iscell);
addParameter(p, 'archs', {'gcn', 'sage', 'gine_linear', 'gine_conv'}, @iscell);
parse(p, varargin{:});
