            ...
            [V_node_padded, C_node_padded, d_node, pred_lb_node, pred_ub_node, ...
             V_edge_padded, C_edge_padded, d_edge, pred_lb_edge, pred_ub_edge] = ...
                obj.pad_to_same_K(V_node_edge, in_gs.C, in_gs.d, in_gs.pred_lb, in_gs.pred_ub, ...
                                  E_proj_V, E_C, E_d, E_pred_lb, E_pred_ub);

            K_node = size(V_node_padded, 3);
            K_edge = size(V_edge_padded, 3);

            V_combined_msg = zeros(numEdges, obj.InputSize, K_node + K_edge - 1, 'like', V_node_padded);
            V_combined_msg(:, :, 1) = V_node_padded(:, :, 1) + V_edge_padded(:, :, 1);
            V_combined_msg(:, :, 2:K_node) = V_node_padded(:, :, 2:end);
            V_combined_msg(:, :, K_node+1:end) = V_edge_padded(:, :, 2:end);
